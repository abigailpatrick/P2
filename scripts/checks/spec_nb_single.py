...
def main():
    ...
    if not override:
        # Exact reproduction: use the stored spectrum AND the stored shifted
        # ra/dec, exactly as sliding_snr_lya_grating.py does, so the pseudo-NB
        # pixel is byte-identical to the pipeline figure.
        ra = float(npz["ra"])
        dec = float(npz["dec"])
        wave = wave_ref
        flux = npz["flux"]
