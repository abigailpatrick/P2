...
def air_to_vac(wave_air):
    """Convert air wavelength(s) in Angstrom to vacuum, Ciddor (1996).

    MUSE cubes are calibrated in air; NIRSpec systemic redshifts are in vacuum.
    The MUSE Lya peak must be moved onto the vacuum system before z_lya is
    differenced against z_sys, otherwise a systematic offset of order 80 km/s
    (the ~2.8e-4 air/vacuum index difference times c) leaks into Delta_v,
    biasing it blueward. The correction is applied at the single point where the
    fitted MUSE peak becomes z_lya, so all internal MUSE steps stay in air.
    """
    wave_air = np.asarray(wave_air, dtype=float)
    s2 = (1e4 / wave_air) ** 2   # squared wavenumber in um^-1
...
def vac_to_air(wave_vac):
    """Inverse of air_to_vac (Ciddor). Used only to place vacuum reference
    wavelengths (systemic Lya, z_lya) correctly over the air-frame MUSE spectrum
    in the plots. The science values are computed in vacuum and never use this.
    """
...
def rebin_for_display(wave, flux, var, nbin):
    """Rebin a spectrum by an integer factor for display only.

    Wavelength is averaged per bin, flux is averaged, and variance is combined
    as the variance of a mean (sum of var / n^2) so the plotted +/-1 sigma band
    drops by ~sqrt(n). Any leftover channels that don't fill a final bin are
    dropped. The fit and all measured quantities are untouched, this affects
    only what is drawn.
    """
    nbin = int(nbin)
    if nbin <= 1:
    ...
    v = var[:n].reshape(-1, nbin)

    with np.errstate(invalid="ignore"):
        wb = np.nanmean(w, axis=1)
        fb = np.nanmean(f, axis=1)
        # variance of the mean of nbin channels
        vb = np.nansum(v, axis=1) / (nbin ** 2)

    return wb, fb, vb


def plot_fit(wave, flux, var, fit, lya_sys, lya_peak, source_id,
             ra, dec, delta_v, outpath, z_sys=None, plot_halfwidth=250.0,
             plot_bin=2):
    """Spectrum with the skewed-Gaussian fit, systemic and Lya-peak lines."""
    # Rebin the drawn spectrum for display only. The fit and every measured
    # quantity already used the full-resolution data.
    wave_d, flux_d, var_d = rebin_for_display(wave, flux, var, plot_bin)
    noise_d = np.sqrt(np.clip(var_d, 0.0, np.inf))

    fig, ax = plt.subplots(figsize=(9, 4))

    ax.step(wave_d, flux_d, where="mid", color="black", lw=1.0, alpha=0.6,
            label="Flux" if plot_bin <= 1 else f"Flux (bin {int(plot_bin)})")
    ax.fill_between(wave_d, -noise_d, noise_d, color="gray", alpha=0.3,
                    step="mid", label=r"$\pm 1\sigma$")
    ...
    in_win = (wave_d >= lya_sys - plot_halfwidth) & (wave_d <= lya_sys + plot_halfwidth)
    if np.any(in_win):
        vals = np.concatenate([flux_d[in_win], noise_d[in_win], -noise_d[in_win]])
    ...
    title = f"Source {source_id} | RA={ra:.5f} | Dec={dec:.5f}"
    if z_sys is not None and np.isfinite(z_sys):
        title += rf" | $z_{{\rm sys}}={z_sys:.4f}$"
    ax.set_title(title)
...
def parse_args():
    ...
    p.add_argument("--plot-bin", type=int, default=2,
                   help="Rebin the plotted spectrum by this integer factor for "
                        "display only (default 2). The fit and all measured "
                        "quantities always use the full-resolution data. Set 1 "
                        "for no binning.")
...
def main():
    ...
    for path in files:
        ...
        if not args.no_plots:
            ...
            # The drawn spectrum is in air. Place the systemic marker at its
            # air-equivalent wavelength; the Lya-peak marker is already air.
            lya_sys_air = vac_to_air(lya_sys)
            plot_fit(wave, flux, var, fit, lya_sys_air, lya_peak, idx,
                     ra, dec, dv, outpng, z_sys=z_sys,
                     plot_halfwidth=args.plot_halfwidth,
                     plot_bin=args.plot_bin)
